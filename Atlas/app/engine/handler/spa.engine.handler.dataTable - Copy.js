define(["jquery",
    ...
    function (e, t, n) {
        n.load("/DesktopModules/Atlas/app/engine/infrastructure/dataTable/spa.engine.infrastructure.dataTable.css"),
            t.bindingHandlers.dataTable = {
                init: function (n, a, all, vm) {
                    var r = t.unwrap(a());
                    r.element = n,
                    ...
                    // باز و بسته کردن هر سطر جدول
                    e(r.element).on('click', '.btndetail', function () {
                        var parent = e(this).closest('tr');
                        var rowdata = r.current.row(parent).data();
                        vm.selectedId(rowdata.ID);
                        vm.edit();
                    });
                }
            }
